...
| D14 | **Zero committed secrets** | SQLite needs none; Azure uses Managed Identity (no SQL password exists); deploys use GitHub OIDC (no stored deploy credential). Placeholder-only docs; scripts discover values dynamically (env vars -> az/gh CLI -> tags) and never echo them. *Implementation note:* Key Vault is **not** provisioned by default - after Managed Identity and OIDC the only secret left is the optional Socrata token, which the provisioning script stores as an App Service application setting (encrypted at rest). `-WithKeyVault` moves it into a vault referenced by that setting, for demonstration rather than necessity |
